...
plt = Plot.plot({
    ...
    marks: [
      Plot.ruleY([0]),
      Plot.ruleX([0]),
      Plot.line(data, {x: "x", y: "pdf", stroke : "type", strokeWidth: 2})
    ]
  })
...
data = {
  ...
  const mysinval = x.map(x => ({x: x, pdf: Math.sin(- x * mu ), type: "sin"}));
  const mycosval = x.map(x => ({x: x, pdf: Math.cos(- x * mu ), type: "cos"}));
