...
This naturally can be encoded as sparse binary vectors.

An example with `pandas` and  `sklearn` is given below, using the usual convention that `0` represents `False` and `1` represents `True`:
...
categorical_column = ['Name', 'Embarked']
...
print(pd.get_dummies(df_titanic_raw.Embarked))

transformer=make_column_transformer((OneHotEncoder(sparse_output=False), ['Embarked']), remainder="passthrough")
df_new = transformer.fit_transform(df_titanic_raw)
print(df_new)
```
::: {.callout-note}
The difference in the output created lies in the way the missing values are treated.
:::
