...
- **Mid-term** project snapshot: Due date  **November 15, 23:59**. This part consists of starting the **group project**, showing preliminary work and organization of the workload, git first steps for the group project. The groups will be available on [Moodle](https://moodle.umontpellier.fr/course/view.php?id=22454). This part corresponds to **10% of the final grade**. The due date is **October 16 (23:59)**

- Due date (final project): The GitHub repository should be completed before **Tuesday 6 December (23:59)**. Nothing pushed after the deadline will be taken into account. The oral presentation (max: 20mn) The due date is **December 12 (23:59)** (room: TBD).

### Mid-term project snapshot

- Create a minimum viable project, showing the architecture (website, files, classes, etc.). It does not have to be functional at this stage but must have the main files needed, details of the coding pipeline, choice of the packages/technologies used, etc.
...
- Retro planning with a Gant diagram as follows (see for instance [Quarto](https://quarto.org/docs/authoring/diagrams.html) and [Mermaid](https://mermaid.js.org/syntax/gantt.html)):
...
- The ultimate goal is to provide a Python module that can be imported with `pip` and contains your work, so a new user could re-use all your work.
A description of the procedure will be needed (imagine you are addressing a user not aware of your package).
...
|General |Details|Points (out of 20)|
|--------|--------------|-------|
|**Preliminary-defense**|Slides (structure, spelling)|  1           |
