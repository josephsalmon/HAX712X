...
# This is a `magic` command, that works only in interactive cases (Ipython, Notebook, Jupyter lab etc.)
...
- [Introduction to Sparse Matrices in Python with SciPy](http://cmdlinetips.com/2018/03/sparse-matrices-in-python-with-scipy), by by cmdlinetips
...
Create a linear model that can handle sparse matrices, in particular, how would you do the usual pre-processing step of standardizing the columns of the design matrix?
...
ox.plot_graph(G)
```
## Visualize shortest path between two points
**References**:
...
Create a matrix of similar size. BEWARE CREATE HUGE MATRIX:
...
Useful shortcuts are available (*e.g.,* touche c, touche j etc.), with at full list is [here](https://docs.python.org/3/library/pdb.html).s
...
`pdb`. A terminal is launched when a problem occurs, and one can then take over and see what's going on.
