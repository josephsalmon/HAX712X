...
The main task is to provide a description of the cost distribution for each section of the highway: it does depend on where you enter and get out. For instance,
...
- It appears to be cheaper to exit and re-enter the highway during a trip than tos go straight from the initial point to the final destination... Thence, provide an algorithm that computes the minimal fees to go from one exit to another with the constraint k maximum in/out.
