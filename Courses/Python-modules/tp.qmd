...
See the git repo of the course in the subfolder [`Courses/Python-modules/modules_files`](https://github.com/josephsalmon/HAX712X/tree/main/Courses/Python-modules/modules_files): it contains files ou should copy into your package tree:
  ...
  5. Populate the `io` sub-module with the file `Load_db.py` (it downloads the bike data-set). At the loading step of the `io` sub-module, it should create the variables (*Hint: this is in the `__init__.py` file*)
:::
...
## EXERCISE: 
  ...
  1. Be sure to have the following packages installed (*Hint: this list could be saved in a `requirements.txt` file at the project root folder.*):
```bash
tqdm
pygal_maps_fr
pooch
...
$ python script.py
```
    It should open a navigator and display:

![Biketrauma map](biketrauma_map.png)
:::
