...
- [super user: shell, console and terminal](https://superuser.com/questions/144666/what-is-the-difference-between-shell-console-and-terminal)
...
- [The Linux Directory Structure, Explained](https://www.howtogeek.com/117435/htg-explains-the-linux-directory-structure-explained/), by Chris Hoffman.
...
**References**: 
[What are the differences between most, more and less?](https://unix.stackexchange.com/questions/81129/what-are-the-differences-between-most-more-and-less) on StackExchange.
...
Obiously, replace `target_path` and `link_path` by their corresponding values.
...
- [File system permission](https://en.wikipedia.org/wiki/File_system_permissions) on Wikipedia. See also `chown` and `chgrp`.
...
Many options of bash may be changed with `envs`.
...
- [How To Read and Set Environmental and Shell Variables on Linux](https://www.digitalocean.com/community/tutorials/how-to-read-and-set-environmental-and-shell-variables-on-a-linux-vps) by Justin Ellingwood.
...
References: [Tutorials Point: Unix/Linux - Process management](https://www.tutorialspoint.com/unix/unix-processes.html)
...
The dataset we are going to use is available on the [data.gouv.fr](https://www.data.gouv.fr/fr/datasets/accidents-de-velo-en-france/) plateform. We will focus on bicycles accident in France from 2005-2017.
...
## EXERCISE: grep
   ...
   1. Count the number of accidents in 2005 using the command `grep` (hint: remarks that each line **starts** with the string `"YYYY` where `YYYY` is the year)
...
[TecMint](https://www.tecmint.com/35-practical-examples-of-linux-find-command/), 35 Practical Examples of Linux Find Command
...
**References**:
- [Piping and Redirection!](https://ryanstutorials.net/linuxtutorial/piping.php) by Ryan Chadwick.
...
-   `sed`, `awk` programs and the `perl` language. XXX
