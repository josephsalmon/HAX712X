...
## Details for the personal project: a weather forecast app
...
and then provide the URL here:
[https://framaforms.org/hax712x-depot-git-personnel-1693995810](https://framaforms.org/hax712x-depot-git-personnel-1693995810).


For this project you have to create a simple webpage with quarto that can update the weather forecast in an automatic way, to display for instance something like this:

![Weather Forecast](meteo.png){width=500}

::: {.callout-note}
The closer you can reproduce something in the spirit of the previous illustration, the higher the grade.
:::

An additional constraint is that the app should be autonomous once created, and could refresh every day automatically.
An example to help you is available [here](), as well as the [associated source code](https://github.com/josephsalmon/Tweets/blob/master/economics/index.qmd).
This result is obtained using a GitHub page with GitHub action with quarto; see [https://quarto.org/docs/publishing/github-pages.html](https://quarto.org/docs/publishing/github-pages.html).

For the automatic update see the [schedule](https://docs.github.com/en/actions/using-workflows/events-that-trigger-workflows#schedule) event description.


To automatize the data download, you can use the following website:
...
## Intermediate tasks
 - data **cleaning steps**: removing missing salues if need be.
