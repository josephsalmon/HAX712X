...
In the future, they will be treated as `NA` (note available), in a similar way as in R; see the [Pandas documentation on missing data](https://pandas.pydata.org/pandas-docs/stable/user_guide/missing_data.html) for standard behavior and details.
...
Note that we use here the following conventions:
 - d = day
 - m=month
 - Y=year
 - H=hour
...
In June 2023, the author decides to change the name of the columns... We define a dictionary to come back to legacy names...
...
time_improved = pd.to_datetime(df_bikes['date'] +
                               ' ' + df_bikes['heure'],
                               format='%Y-%m-%d %H:%M',
                               errors='coerce') # Warning : In June 2023, there is badly formated times...
...
- [Notebook on French departements issues](https://www.kaggle.com/code/scratchpad/notebook670ec82922/edit)
