...
ax = sns.kdeplot(df_titanic['Age'],
                 fill=True,
                 cut=0,
                 bw_adjust=0.1)  # bw: bandwith
...
### <font color='red'> EXERCISE : sex ratio on the boat </font>
...
### <font color='red'> EXERCISE : Accidents by department </font>
...
liste_dp = df_dtp_pop.index

gd = df_bikes.groupby(['departement']).size()

data = {'code': gd.index,
        'n_accidents': gd.values}
df = pd.DataFrame(data)
df['code'] = df['code'].apply(lambda x: str(x).zfill(3))
df.sort_values(by=['code'], inplace=True)
df['code'] = liste_dp
df['n_accidents'] = df['n_accidents'].values * 10000./ df_dtp_pop['Population'].values
